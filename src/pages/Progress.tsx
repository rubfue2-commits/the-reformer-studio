  ...
  return (
    <MobileLayout>
      <div className="px-6 pt-14">
        ...
        <div className="mb-5">
          <p className="font-body text-xs tracking-widest uppercase text-muted-foreground mb-1">Connect Reformer</p>
          <h1 className="font-display text-3xl font-light text-foreground">{t("Progrès", "Progress")}</h1>
        </div>
          ...
          {METRICS.map(m => (
            <button key={m.key} onClick={() => setActiveMetric(m.key)}
              className={"flex-shrink-0 px-4 py-2 rounded-full font-body text-xs font-medium transition-all border " +
                (activeMetric === m.key ? "bg-foreground text-background border-foreground" : "bg-card border-border text-muted-foreground")}>
              {t(m.fr, m.en)}
            </button>
          ))}
        </div>
            ...
            <div className="space-y-2">
              {metricData.slice(0, 10).map((m: any, i: number) => (
                <div key={i} className="flex items-center justify-between bg-card border border-border rounded-xl px-4 py-3">
                  <p className="font-body text-xs text-muted-foreground">{new Date(m.measured_at).toLocaleDateString()}</p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50" onClick={() => setShowModal(false)}>
          <div className="bg-background rounded-t-3xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <p className="font-display text-xl font-light text-foreground mb-1">{t("Ajouter une mesure", "Add measurement")}</p>
            ...
            <input type="number" value={newValue} onChange={e => setNewValue(e.target.value)}
              placeholder={t("Valeur...", "Value...")} autoFocus
