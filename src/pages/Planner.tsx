...
const generateWeek = (startDate: Date) => {
  return daysOfWeek.map((day, i) => {
    ...
    return { day, date: date.getDate(), fullDate: date };
  });
};
    ...
    <MobileLayout>
      <div className="px-6 pt-14">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <h1 className="font-display text-3xl font-light text-foreground">{t("Planning", "Planner")}</h1>
          <p className="mt-1 font-body text-sm text-muted-foreground">{t("Ce mois-ci", "This month")}</p>
        </motion.div>
          ...
          {week.map(({ day, date }) => {
            ...
            return (
              <button key={day} onClick={() => setSelectedDay(date)}
                className={`relative flex flex-1 flex-col items-center gap-1 rounded-2xl py-3 transition-all ${isSelected ? "bg-primary text-primary-foreground" : "bg-card text-foreground"}`}>
                <span className={`font-body text-[10px] ${isSelected ? "text-primary-foreground/60" : "text-muted-foreground"}`}>{day}</span>
                <span className="font-display text-lg">{date}</span>
                {isCompleted && !isSelected && <Check size={10} className="text-gold" />}
                {hasWorkout && !isCompleted && <div className="h-1 w-1 rounded-full bg-gold" />}
              </button>
            );
          })}
        </div>
          ...
          {scheduledWorkouts[String(selectedDay)] ? (
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              <button className="mt-4 w-full rounded-xl bg-primary py-3 font-body text-sm font-medium text-primary-foreground">
                {t("Commencer la séance", "Start session")}
              </button>
            </div>
          ) : (
            <div className="rounded-2xl border border-dashed border-border p-8 text-center">
              <p className="font-body text-sm text-muted-foreground">{t("Aucune séance prévue", "No session planned")}</p>
              <button className="mt-3 font-body text-xs text-gold underline decoration-gold/30 underline-offset-2">{t("Ajouter une séance", "Add a workout")}</button>
            </div>
          )}
        </motion.div>
