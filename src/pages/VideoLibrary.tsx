            ...
            <motion.div key="sessions" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-5">
              ...
              {sessions.length === 0 ? (
                <div style={{ textAlign: "center", padding: "60px 20px" }}>
                  ...
                  <p style={{ fontSize: 15, fontWeight: 600, color:"hsl(var(--foreground))", marginBottom: 4 }}>{t("Séances à venir", "Upcoming sessions")}</p>
                  <p style={{ fontSize: 13, color:"hsl(var(--muted-foreground))", lineHeight: 1.5 }}>{t("Les vidéos de ce programme sont en cours de production.", "Videos for this program are being produced.")}</p>
                </div>
              ) : (
                  ...
                  {sessions.map((session, i) => {
                    ...
                    return (
                      <motion.div key={session.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                        ...
                        <div style={{ height: 3, backgroundColor: locked ? "hsl(var(--border))" : selectedCat.color }} />
                        <div className="p-4">
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 4 }}>
                                ...
                                <button
                                  ...
                                  style={{ width: 34, height: 34, borderRadius: "50%", backgroundColor: locked ? "hsl(var(--border))" : selectedCat.color, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "none", cursor: locked ? "default" : "pointer", padding: 0 }}>
      ...
      <AnimatePresence>
        {showFilters && (
          <motion.div key="filters" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} transition={{ type: "tween", duration: 0.22 }}
            ...
            {/* Header */}
            <div style={{ display:"flex", alignItems:"center", gap:12, padding:"56px 20px 14px", backgroundColor:"hsl(var(--card))", borderBottom:"1px solid hsl(var(--border))" }}>
              ...
              <button onClick={resetFilters}
                style={{ width:38, height:38, borderRadius:"50%", backgroundColor: activeFilterCount ? "#B8973E" : "hsl(var(--border))", border:"none", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", flexShrink:0 }}>
                <RotateCcw size={16} color={activeFilterCount ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))"}/>
              </button>
            </div>
            ...
            <div style={{ flex:1, overflowY:"auto", padding:"18px 20px 120px" }}>
              ...
              {!showResults && (<>
                {/* Recherche */}
                <div style={{ display:"flex", alignItems:"center", gap:10, backgroundColor:"hsl(var(--card))", borderRadius:999, padding:"13px 18px", marginBottom:24, boxShadow:"0 1px 6px rgba(0,0,0,0.04)" }}>
                  <Search size={17} color="hsl(var(--muted-foreground))"/>
                  <input value={fSearch} onChange={e => setFSearch(e.target.value)} placeholder="Recherche"
                    style={{ flex:1, border:"none", outline:"none", fontSize:15, color:"hsl(var(--foreground))", backgroundColor:"transparent", fontFamily:"inherit" }}/>
                  {fSearch && <button onClick={() => setFSearch("")} style={{ background:"none", border:"none", cursor:"pointer", padding:0 }}><X size={15} color="hsl(var(--muted-foreground))"/></button>}
                </div>
                ...
                {([
                  ...
                  <div key={title} style={{ marginBottom:26 }}>
                    <p style={{ fontSize:17, fontWeight:700, color:"hsl(var(--foreground))", marginBottom:12 }}>
              ...
              {showResults && (
                filtered.length === 0 ? (
                  <div style={{ textAlign:"center", padding:"60px 20px" }}>
                    ...
                    <p style={{ fontSize:15, fontWeight:600, color:"hsl(var(--foreground))", marginBottom:4 }}>{t("Aucune séance", "No session")}</p>
                    <p style={{ fontSize:13, color:"hsl(var(--muted-foreground))" }}>{t("Essaie d'élargir tes filtres.", "Try broadening your filters.")}</p>
                    <button onClick={() => setShowResults(false)} style={{ marginTop:16, padding:"11px 22px", borderRadius:999, border:"none", backgroundColor:"hsl(var(--foreground))", color:"hsl(var(--background))", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>{t("Modifier les filtres", "Edit filters")}</button>
                  </div>
                ) : (
                    ...
                    {filtered.map((s, i) => {
                      ...
                      return (
                        <motion.button key={s.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                          ...
                          style={{ textAlign:"left", backgroundColor:"hsl(var(--card))", borderRadius:20, padding:16, border:"1px solid hsl(var(--border))", boxShadow:"0 1px 8px rgba(0,0,0,0.04)", cursor:"pointer", fontFamily:"inherit" }}>
                          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, marginBottom:6 }}>
                            <p style={{ fontSize:15, fontWeight:600, color:"hsl(var(--foreground))", margin:0 }}>{s.name_fr}</p>
                            <div style={{ width:32, height:32, borderRadius:"50%", backgroundColor: locked ? "hsl(var(--border))" : (cat?.color || "#B8973E"), display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                              {locked ? <Lock size={12} color="hsl(var(--muted-foreground))"/> : <Play size={12} color="white" fill="white"/>}
                            </div>
                          </div>
                          <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
