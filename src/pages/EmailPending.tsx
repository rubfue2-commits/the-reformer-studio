    ...
    <div style={{ minHeight: "100dvh", backgroundColor: "#F5F3EE", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 24px" }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div style={{ backgroundColor: "white", borderRadius: 24, padding: "36px 24px", boxShadow: "0 4px 32px rgba(0,0,0,0.06)", marginBottom: 16 }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 700, color: "#1C1B19", margin: "0 0 8px" }}>Confirme ton email</h2>
          <p style={{ fontSize: 14, color: "#6B6560", lineHeight: 1.6, margin: "0 0 8px" }}>Un email de confirmation a été envoyé à</p>
          ...
          {sent ? (
            ...
            <button onClick={resend} disabled={loading}
              ...
              <RefreshCw size={15} />{loading ? "Envoi..." : "Renvoyer l'email"}
            </button>
          )}
        ...
        <button onClick={signOut} style={{ background: "none", border: "none", color: "#8B8578", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Se déconnecter</button>
      </motion.div>
    </div>
  );
}